...
df_benign_def = pd.read_csv('definitive_benign.csv').dropna(subset=['url'])
df_benign_def['label'] = 'Normal'
print(f"   ★ Benign: {len(df_benign_def)} URLs")

# Malicious Pool (Active Phishing Database)
df_m_phish = pd.read_csv('definitive_malicious.csv').dropna(subset=['url'])
df_m_phish['label'] = 'Phishing'
print(f"   ★ Malicious: {len(df_m_phish)} URLs")

# PhishTank
df_p = pd.read_csv('phishtank.csv').dropna(subset=['url'])
df_p['label'] = 'Phishing'
...
df_unseen = pd.read_csv('new_unseen_dataset.csv').dropna()
...
cnn.save('local_hybrid_model.keras')
joblib.dump(tokenizer, 'local_tokenizer.pkl')
joblib.dump(scaler, 'local_url_scaler.pkl')
joblib.dump(xgb_model, 'local_meta_learner_global.pkl')
